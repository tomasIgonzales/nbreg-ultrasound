...
// Run regression model for each machine-machine combo, leaving out one PID and then predicting in that PID
...
foreach Out of local Machine_List{
    ...
    foreach Pred in `s1' `s2'{
        ...
        foreach Var in lnmean lnalpha{
            ...
            lincom _b[_cons] + _b[2.Muscle]
            lincom _b[_cons] + _b[3.Muscle]
            lincom _b[_cons] + _b[4.Muscle]
            lincom _b[_cons] + _b[5.Muscle]
